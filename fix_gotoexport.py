...
robust_function = """    function goToExport() {
      let previewImageURL = '';
      try {
      ...
      // Special handling for furniture specifics
...
pattern1 = re.compile(r"function\s+goToExport\(\)\s*\{[\s\S]*?window\.location\.href\s*=\s*'export_area\.html';\s*\n?\s*\}", re.MULTILINE)
pattern2 = re.compile(r"function\s+goToExport\(\)\s*\{[\s\S]*?renderer\.render\(scene, camera\);[\s\S]*?window\.location\.href\s*=\s*'export_area\.html';\s*\n?\s*\}", re.MULTILINE)
pattern3 = re.compile(r"function\s+goToExport\(\)\s*\{[^\}]+\}", re.MULTILINE)

for filename in files_to_fix:
    path = os.path.join(r"c:\CASE STUDY\FurniCraft", filename)
    if os.path.exists(path):
        ...
        if matches:
            ...
            new_content = content.replace(first_match, robust_function, 1)
            for other_match in matches[1:]:
                new_content = new_content.replace(other_match, "")
            
            with open(path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Fixed {filename} (found {len(matches)} occurrences)")
        else:
            print(f"No goToExport() found in {filename} to replace!")
